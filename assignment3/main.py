...
with tf.Session() as sess:
    ...
    sess.run(init)
    validate_feed={x:train[:1999,:56],y_:train[:1999,56:]}
    dev_feed={x:train[1999:,:56],y_:train[1999:,56:]}
    ...
    saver.save(sess,'/home/lee/assignment3/saver/model.ckpt')
    dev_acc=sess.run(accuracy,feed_dict=dev_feed)
    print("After %d training step(s),dev accuracy using average model is %g" % (TRAINING_STEPS, dev_acc))
